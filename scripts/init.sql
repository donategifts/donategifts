...
CREATE OR REPLACE TRIGGER SET_UPDATED_AT 
	SET_UPDATED_AT SET_UPDATED_AT before
	update
		on public.verifications for each row
	execute
		procedure public.TRIGGER_SET_UPDATED_DATE();
...
CREATE OR REPLACE TRIGGER SET_UPDATED_AT 
	SET_UPDATED_AT SET_UPDATED_AT before
	update
		on public.agencies for each row
	execute
		procedure public.TRIGGER_SET_UPDATED_DATE();
...
CREATE OR REPLACE TRIGGER SET_UPDATED_AT 
	SET_UPDATED_AT SET_UPDATED_AT before
	update
		on public.wishcards for each row
	execute
		procedure public.TRIGGER_SET_UPDATED_DATE();
