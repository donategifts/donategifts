...
export default class Messaging {
    ...
    static async getTransport() {
        if (config.NODE_ENV === 'development' || config.NODE_ENV === 'test') {
            ...
            return nodemailer.createTransport({
                // for some reason typescript can't infer this although it's in the type definition
                // @ts-ignore
                host: 'smtp.gmail.com',
                port: 587,
                auth: {
    ...
    static sendWishPublishedEmail({ agencyEmail, childName }: { agencyEmail: string; childName: string }) {
    ...
    static sendVerificationEmail(to: string, hash: string) {
    ...
    static sendPasswordResetMail(to: string, hash: string) {
    ...
    static async sendDiscordDonationNotification({
        ...
        user: string;
        service: string;
        wishCard: { item: string; url: string; child: string };
        donation: { amount: number; userDonation: number };
    }) {
